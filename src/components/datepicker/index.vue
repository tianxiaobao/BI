    ...
    <input
        ...
        <div class="calendar" v-show="showDayView" v-bind:style="calendarStyle">
            ...
            --><span class="cell day"
                v-for="day in days"
                track-by="timestamp"
                v-bind:class="{ 'selected':day.isSelected, 'disabled':day.isDisabled, 'today': day.isToday }"
...
export default {
  ...
  methods: {
    ...
    setDate (timestamp) {
      ...
      this.$emit('selected', new Date(timestamp), this)
    ...
    today (dObj) {
        const d = new Date()
        return (d.getFullYear() == dObj.getFullYear()) && (d.getMonth() == dObj.getMonth()) && (d.getDate() == dObj.getDate())
    },
    /**
