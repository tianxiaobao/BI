<style type="text/css">
    ...
    .s-leftMenu{top:112px;}
    .source-menu li:hover span.fr, .source-menu li.on:hover span.tips-text{display: block;}
    ul.this span.fr,ul.this li.on span.tips-text{display:none;}
</style>
<template>
    <div id="datasource">
        <div class="s-leftMenu ml2">
            ...
            <div class="mc">
                ...
                <ul class="source-menu overflow-ul this">
                    <li v-for="item in datasourceDisplay"
                        ...
                        <span class="tips-text fr" style="z-index: 10;">
                            <b @click="deleteDatasource(item.dbId, item.isUse, item.displayName)">
                                删除
                            </b>
                            <span class="tips-con visit-con">名称：{{ item.displayName }}<br>所有者：{{ item.owner }}</span>
                        </span>
                        <div class="clear"></div>
                    </li>
                </ul>
...
export default {
    ...
    methods: {
        ...
        getOptionList() {
            api.datasource.getAllDatasource().then((response) => {
                if(response.code == 0) {
                    ...
                    let msg = response.message || '获取下拉框列表失败';
                    this.layerMsg(msg);
                }
            })
            .catch(function (error) {
        ...
        toggleSubmitList(item) {
            ...
            let index = this.submitList.indexOf(item.dbName);
            if(item.checked) {
